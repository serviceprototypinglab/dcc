...
def validate(prefix, idx, metrics, props, filenames):
    ...
    with open(f'result/{prefix}_outliers.json', 'w') as outlier_file:
        json.dump(outlier_json, outlier_file)
    df.to_csv(f'result/{prefix}_concat_no_outliers.csv')
    ...
    result.to_csv(f'result/{prefix}_aggregate.csv')
    df.to_csv(f'result/{prefix}_mean.csv')
